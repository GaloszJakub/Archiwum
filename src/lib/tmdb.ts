...
class TMDBService {
    ...
    async getUpcomingMovies(page: number = 1): Promise<SearchResponse> {
        const cacheKey = `tmdb_upcoming_movies_${page}`;
        const cached = this.getFromCache(cacheKey);
        if (cached) return cached;

        const data = await this.fetch<SearchResponse>('/movie/upcoming', {
            page: page.toString(),
            region: 'PL' // Recommend movies coming to Poland
        });
    ...
    async getTopRatedMovies(page: number = 1): Promise<SearchResponse> {
        const cacheKey = `tmdb_top_rated_movies_${page}`;
        const cached = this.getFromCache(cacheKey);
        if (cached) return cached;

        const data = await this.fetch<SearchResponse>('/movie/top_rated', {
            page: page.toString(),
        });
    ...
    async getOnTheAirTVShows(page: number = 1): Promise<SearchResponse> {
        const cacheKey = `tmdb_ontheair_tv_${page}`;
        const cached = this.getFromCache(cacheKey);
        if (cached) return cached;

        const data = await this.fetch<SearchResponse>('/tv/on_the_air', {
            page: page.toString(),
        });
    ...
    async getTopRatedTVShows(page: number = 1): Promise<SearchResponse> {
        const cacheKey = `tmdb_top_rated_tv_${page}`;
        const cached = this.getFromCache(cacheKey);
        if (cached) return cached;

        const data = await this.fetch<SearchResponse>('/tv/top_rated', {
            page: page.toString(),
        });
    ...
    private getFromCache(key: string): any | null {
        try {
            ...
            if (now - timestamp < 60 * 60 * 1000) { // 1 hour
                return data; // Changed from return cached to return data
            }

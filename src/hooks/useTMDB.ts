...
export const useUpcomingMovies = () => {
    return useInfiniteQuery({
        ...
        getNextPageParam: (lastPage) => {
            return lastPage.page < Math.min(lastPage.total_pages, 3) ? lastPage.page + 1 : undefined;
        },
        initialPageParam: 1,
        staleTime: 60 * 60 * 1000,
        gcTime: 2 * 60 * 60 * 1000,
...
export const useTopRatedMovies = () => {
    return useInfiniteQuery({
        queryKey: ['tmdb', 'top_rated', 'movies'],
...
export const useOnTheAirTVShows = () => {
    return useInfiniteQuery({
        queryKey: ['tmdb', 'ontheair', 'tv'],
        queryFn: ({ pageParam = 1 }) => tmdbService.getOnTheAirTVShows(pageParam),
        getNextPageParam: (lastPage) => {
            return lastPage.page < Math.min(lastPage.total_pages, 3) ? lastPage.page + 1 : undefined;
        },
        initialPageParam: 1,
...
export const useTopRatedTVShows = () => {
    return useInfiniteQuery({
        queryKey: ['tmdb', 'top_rated', 'tv'],
        queryFn: ({ pageParam = 1 }) => tmdbService.getTopRatedTVShows(pageParam),
        getNextPageParam: (lastPage) => {
            return lastPage.page < Math.min(lastPage.total_pages, 3) ? lastPage.page + 1 : undefined;
        },
        initialPageParam: 1,
